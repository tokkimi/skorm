...
export default async function AdminArtistsPage() {
  ...
  return (
    <main className="admin-main">
      <AdminPageHeading title="Artistes" description="Profils, informations professionnelles, médias, tarifs et disponibilités." action="+ Ajouter un artiste" />
      ...
      <section className="artist-admin-grid">
        {data.map((artist) => (
          <article className="artist-admin-card" key={artist.id}>
            ...
            <footer><span>Instagram</span><b>{artist.instagram_url ? "Connecté" : "À connecter"}</b><AdminItemActions kind="artist" item={artist} /></footer>
          </article>
        ))}
      </section>
    </main>
