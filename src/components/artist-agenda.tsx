...
type Entry = { id: string; title: string; starts_at: string; ends_at?: string | null; location: string; notes: string; visibility: "private" | "public" };
...
export function ArtistAgenda({ action }: { action: "agenda" | "date" | "notes" }) {
  ...
  async function load(signal?: AbortSignal) {
    ...
    setEntries(data.events); setNote(data.note); setLoaded(true);
  }
  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/artist/agenda", { cache: "no-store", signal: controller.signal })
      .then(async response => { const data = await response.json(); if (!response.ok) throw new Error(data.error); return data; })
      .then(data => { setEntries(data.events); setNote(data.note); setLoaded(true); })
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (await send({ action: "event", id: editing?.id, visibility, title: form.get("title"), starts_at, ends_at, location: form.get("location"), notes: form.get("notes") || "" })) {
  ...
  const selectedEntries = entries.filter(e => dayKey(new Date(e.starts_at)) === selected).sort((a,b)=>a.starts_at.localeCompare(b.starts_at));
  ...
  return <section className="artist-agenda">
    ...
    {loaded && action !== "notes" && <>
      ...
      <h3>{new Date(`${selected}T12:00`).toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long"})}</h3>
      {!selectedEntries.length && <p>Aucune date ce jour.</p>}
      {selectedEntries.map(e=><article className="artist-agenda-entry" key={e.id}><small>{e.visibility === "private" ? "🔒 Privé" : "Public"} · {new Date(e.starts_at).toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}</small><h3>{e.title}</h3><p>{e.location}</p>{e.notes && <p style={{whiteSpace:"pre-wrap"}}>{e.notes}</p>}{e.visibility === "private" && <div><button disabled={busy} onClick={()=>{setEditing(e);setVisibility("private");setShowForm(true);}}>Modifier</button><button disabled={busy} onClick={()=>{if(window.confirm("Supprimer ce rendez-vous privé ?")) void send({action:"delete",id:e.id});}}>Supprimer</button></div>}</article>)}
      <div className="artist-agenda-actions"><button onClick={()=>{setEditing(null);setVisibility("private");setShowForm(true);}}>+ Rendez-vous privé</button><button onClick={()=>{setEditing(null);setVisibility("public");setShowForm(true);}}>+ Date publique</button></div>
      {showForm && <form key={editing?.id || `${visibility}-${selected}`} className="artist-private-fields" onSubmit={save}>
        <h3 className="wide">{visibility === "public" ? "Publier une date sur ma page" : "Rendez-vous privé"}</h3>
        ...
        <button disabled={busy}>{busy ? "Enregistrement…" : visibility === "public" ? "Publier cette date" : "Enregistrer"}</button><button type="button" onClick={()=>setShowForm(false)}>Annuler</button>
      </form>}
    </>}
