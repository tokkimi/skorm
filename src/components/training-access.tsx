...
export function TrainingAccess({ sessionId }: { sessionId?: string }) {
  ...
  if (checking) return <main className="training-page"><section className="training-hero"><p>Vérification du paiement...</p></section></main>;
  if (!paid) {
    return (
      <main className="training-page">
        <section className="training-hero">
          ...
          <p>Si tu viens de payer, réouvre le lien de confirmation Stripe. Sinon, retourne à la page formation.</p>
  ...
  return (
    <main className="training-page">
      <section className="training-hero">
        <p className="eyebrow">Formation débloquée</p>
      ...
      <section className="quiz-board">
        {moduleQuizzes.map((quiz) => (
          <article className="quiz-card glass-panel" key={quiz.moduleId}>
            <h2>{quiz.title}</h2>
            <ol>{quiz.questions.map((question) => <li key={question}>{question.replace(/^\d+\. /, "")}</li>)}</ol>
          </article>
        ))}
        <article className="quiz-card final glass-panel">
          ...
          <ol>{finalQuiz.map((question) => <li key={question}>{question.replace(/^\d+\. /, "")}</li>)}</ol>
        </article>
      </section>
    </main>
  );
