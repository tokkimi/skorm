alter table public.artists
  ...
  add column if not exists media_sounds jsonb default '[]'::jsonb,
  add column if not exists media_releases jsonb default '[]'::jsonb,
  add column if not exists media_videos jsonb default '[]'::jsonb;
...
begin
  ...
  if p_kind = 'artist' then
    insert into public.artists(
      ...
      is_active, display_order, media_sounds, media_releases, media_videos
    )
    values (
      p_payload->>'slug',
      p_payload->>'name',
      ...
      nullif(p_payload->>'home_image_url', ''),
      p_payload->'featured_sound',
      true,
...
begin
  ...
  if p_kind = 'artist' then
    update public.artists set
      ...
      home_image_url = nullif(p_payload->>'home_image_url', ''),
      featured_sound = p_payload->'featured_sound',
